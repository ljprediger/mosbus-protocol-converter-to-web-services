...
/* free allocated plugin data. If fcopy() is not set, then this function is not called for all copies of the plugin
created with soap_copy(). In this example, the fcopy() callback can be safely omitted. When omitted,
the plugin data is shared by the soap copies created with soap_copy() */
	vPortFree (p->data);
}
...
{
  //WS4D_UNUSED_PARAM (src); Ver si genera warning no usar un parámetro

  return lwip_plugin_init (soap, dst, NULL);
}

static void lwip_plugin_setio (struct soap *soap);
/*en los ejemplos y user guide ponen el 2º arg como struct plugin_data pero en los ejemplos
convierte  p->data = (void*)malloc(sizeof(struct plugin_data)); y plugin_init(soap, (struct plugin_data*)p->data) y ver porque en otros ejemplos el
prototipo de init:
static int plugin_init(struct soap *soap, struct plugin_data *data);, o sea tiene un arg menos*/
static int
lwip_plugin_init (struct soap *soap, struct soap_plugin *p, void *arg)
{
  ...
  p->id = lwip_plugin_id;
  p->data = (void *) pvPortMalloc (sizeof (struct plugin_peer));
...
{
  return (struct plugin_peer *) soap_lookup_plugin (soap, LWIP_PLUGIN_ID);// fetch plugin's local data (look-up)
}
...
struct netif *netif;
//estas líneas están en un else de stdsoap2.cpp
...
//esto está exactamente igual a la de stdsoap2.c[pp] 2.8.0
static int
...
//supongo que será para inicializar con algún callback
int
