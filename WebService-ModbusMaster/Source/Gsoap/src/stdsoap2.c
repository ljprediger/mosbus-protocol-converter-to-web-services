  ...
  { if (soap->d_stream->next_out == Z_NULL)
	...
	r = inflateSetDictionary(soap->d_stream, (const Bytef*)soap->z_dict, soap->z_dict_len);
      ...
      { soap->bufidx = 0;
        ...
        if (r == Z_STREAM_END)
        { DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Inflated ->%lu bytes\n", soap->d_stream->total_in, soap->d_stream->total_out));
...
{ struct soap *soap = (struct soap*)pvPortMalloc(sizeof(struct soap));
  if (soap)
...
{ register void *p = pvPortMalloc(size);
  if (soap)
  { register size_t h = soap_hash_ptr(p);
    register struct soap_mlist *mp = (struct soap_mlist*)pvPortMalloc(sizeof(struct soap_mlist));
...
{ return soap_copy_context((struct soap*)pvPortMalloc(sizeof(struct soap)), soap);
}
#endif
...
{ soap->state = SOAP_INIT;
  ...
  soap->double_format = "%f"; /* Alternative: use "%lG" */
  ...
  if (!soap_ssl_init_done)
  { soap_ssl_init();
    DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Initializing OpenSSL, version=F\n", (long)OPENSSL_VERSION_NUMBER));
  }
...
#endif
    if (s)
    { sprintf(soap->tmpbuf, "%u", (unsigned long)count);
